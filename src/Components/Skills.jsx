...
export default function Skills() {
    const skillsArray = ['XD.svg','UI.svg','IA.svg','UX.svg','Prototyping.svg','Box Model.svg','Business Model Canvas.svg','Desing Systems.svg']
  return<>
  <div className='relative  lg:my-20  lg:h-dvh  '>
    ...
    <h3 className='font-semibold md:text-6xl text-2xl gradiantText'>Skills</h3>
...
<div className='absolute bottom-[98%] left-[40%]'>
...
<stop offset="1" stop-color="#7579FF"/>
    ...
    {/* {large screen} */}
    <div className='lg:grid grid-cols-12 gap-y-10 hidden'>
    {Array(2).fill().map((_, i) => (
        <div key={i} className='col-span-12'>
            <div className='grid grid-cols-2 sm:grid-cols-4 border-t-4 border-[#EDEEFF] lg:gap-x-28 gap-x-10 p-3 gap-y-10 lg:p-10'>
                {skillsArray.slice(i * 4, (i + 1) * 4).map((elem, idx) => (
...
<div className='grid grid-cols-12  lg:hidden'>
    {Array(4).fill().map((_, i) => (
        <div key={i} className='col-span-12'>
            <div className='grid grid-cols-2 border-t-4 border-[#EDEEFF] md:gap-x-28 gap-x-10 p-3 gap-y-10 md:p-10'>
                {skillsArray.slice(i * 2, (i + 1) * 2).map((elem, idx) => (
                    <div key={idx} className='col-span-1'>
                        <img src={elem} className='w-full' alt={elem} />
                    </div>
                ))}
...
<circle cx="123.5" cy="119.5" r="17.5" fill="#EDEEFF"/>
        ...
        <div className='absolute top-[58%] right-0 hidden lg:block'>
